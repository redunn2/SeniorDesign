// Demo created 12/11/16

// The charging robot will move straight for x cm then turn left
// then move straight another y cm to reach sensor node.
...
void moveStraight(float distance /*in cm*/ , float thrust /* fraction between 0 and 1 */) {
  ...
  delay(distance / (20.5*thrust) * 1000);
}
...
void rotateVehicle(float angle, float thrust /* fraction between 0 and 1 */) {
  ...
  delay(angle * 0.077 / (20/thrust) * 1000);
